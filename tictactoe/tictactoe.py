...
def minimax(board):
    ...
    if player(board) == X:
        ...
        for action in actions(board):
            value = Min(result(board,action),-1)
            if(maximum < value):
                Action = action
                maximum = value
    else:
        minimum = 1
        for action in actions(board):
            value = Max(result(board, action),1)
            if(minimum > value):
                Action = action
                minimum = value
    return Action

def Max(board, minsofar):
    if(terminal(board)):
        return utility(board)
    maximum = -1
    for action in actions(board):
        maximum = max(maximum,Min(result(board,action),maximum))
        if maximum >= minsofar:# or maximum ==1:
            return maximum
    return maximum

def Min(board, maxsofar):
    if(terminal(board)):
        return utility(board)
    minimum = 1
    for action in actions(board):
        minimum = min(minimum, Max(result(board, action),minimum))
        if minimum <= maxsofar:
            return minimum
    return minimum
